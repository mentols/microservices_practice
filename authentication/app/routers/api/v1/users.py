...
@api_router.get('/check_core')
async def check_core(authorization: str = Header(None)):
    await TokenServices.decode_access_jwt_token(authorization)
    return Response(status_code=status.HTTP_200_OK)


@api_router.get('/check_statistic')
async def check_statistic(authorization: str = Header(None)):
    # todo: recode for pages.stat
    await TokenServices.decode_access_jwt_token(authorization)
    return Response(status_code=status.HTTP_200_OK)
